...
2. 현재 NMS는 기본 IoU 기반(class-aware/class-agnostic 지원)이며, Soft-NMS/DIoU-NMS 같은 고급 변형은 미포함이다.
3. 클래스 수(예: 120 breed)가 늘어나면 헤드 채널 수가 커져 연산량/메모리 사용이 증가한다.

## 후속 작업
1. IoU 기반 타깃 매칭/할당 전략(예: top-k, SimOTA) 검토
2. mAP 중심 평가 지표 및 리포트 자동화 강화
3. 실제 Stanford Dogs 120 클래스 이름 파일/매핑 고정본 추가
